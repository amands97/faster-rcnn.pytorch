...
from adversary.gumbel import SubsetOperator
...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        self.gumbelTopK = SubsetOperator(14, tau = 0.1, hard = True, device = device)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, apply_mask, maskNet):
        ...
        if self.training and apply_mask == 1:
            ...
            mask = maskNet(pooled_feat)
            maskTensorsize = mask.size()
            mask = mask.view(maskTensorsize[0], -1)
            mask = self.gumbelTopK(mask)
            mask = mask.view(maskTensorsize)
            # mask = self.gumbelTopK(maskNet(pooled_feat))
